...
Timer.prototype.stopTimer = function() {
  ...
  this.toggleElement('toggle', 'Start', 'timer.startTimer()')
};

Timer.prototype.pauseTimer = function() {
  this.toggleElement('toggle', 'Cont...', 'timer.startTimer()')
   for(var t in this.timers) {
    clearInterval(this.timers[t])
  };
};
...
Timer.prototype.setInterval = function(elementName, delay) {
  ...
  this.timers[elementName] = setInterval(function() {
     ...
     if(elementName == 'msec' && self[elementName] > 1000) {
        self[elementName] = 0
        result = 0
     }
     if(elementName != 'msec' && self[elementName] >= 60){
        self[elementName] = 0
        result = 0
     }
     if(result < 10) {
